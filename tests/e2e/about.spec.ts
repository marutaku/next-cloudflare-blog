  ...
  test.describe('About Description', () => {
    ...
    test.describe('SNSアイコン', () => {
      test('GitHub・X(Twitter)のリンクが表示されている', async ({ page }) => {
        // GitHubリンク
        const githubLink = page.locator('a[href="https://github.com/marutaku"]');
        await expect(githubLink).toBeVisible();
        // X(Twitter)リンク
        const xLink = page.locator('a[href="https://twitter.com/Takumi_Maru0131"]');
    ...
    test.describe('肩書き', () => {
      test('肩書きが表示されている', async ({ page }) => {
        ...
        await expect(page.getByText('Software Engineer, NLP Researcher')).toBeVisible();
      });
    });

    test.describe('各セクションの見出し', () => {
      test('職歴・過去の活動・作ったものの見出しが表示されている', async ({ page }) => {
        await expect(page.getByRole('heading', { name: '職歴' })).toBeVisible();
        await expect(page.getByRole('heading', { name: '過去の活動' })).toBeVisible();
        await expect(page.getByRole('heading', { name: '作ったもの' })).toBeVisible();
      });
    });
    ...
    test.describe('各セクションのリスト', () => {
      test('職歴が1件以上表示されている', async ({ page }) => {
        // 職歴の見出しの次に職歴リストが続く想定
        const workItems = await page.locator('h2:text("職歴")').locator('..').locator('div.my-4').count();
        expect(workItems).toBeGreaterThan(0);
      });

      test('過去の活動が1件以上表示されている', async ({ page }) => {
        const activityItems = await page.locator('h2:text("過去の活動")').locator('..').locator('div.my-4').count();
        expect(activityItems).toBeGreaterThan(0);
      });

      test('作ったものが1件以上表示されている', async ({ page }) => {
        const workItems = await page.locator('h2:text("作ったもの")').locator('..').locator('div.my-4').count();
        expect(workItems).toBeGreaterThan(0);
      });
    });
  });
